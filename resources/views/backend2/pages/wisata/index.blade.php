        ...
        <div class="row">
                ...
                <div class="app-action-panel" id="contacts-action-panel">
                    ...
                    <div id="categories-list" class="app-actions-list scrollable-container">
                        <div class="list-group">
                            <a href="{{ route('admin.wisata.index') }}" class="list-group-item">
                        ...
                        <div class="list-group">
                            ...
                            <div class="list-group-item">
                                <div class="item-data" style="width: 100%;" onclick="window.location.href='{{ route('admin.wisata.category', $category->id) }}'">
                            ...
                            <div id="editcategory{{ $category->id }}" class="modal fade" tabindex="-1" role="dialog">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <form action="{{ route('admin.wisata.category.update', $category->id) }}" id="newCategoryForm" method="post">
                                            @csrf
                            ...
                            <div id="deletecategory{{ $category->id }}" class="modal fade" tabindex="-1" role="dialog">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            <a href="{{ route('admin.wisata.category.delete', $category->id) }}"" class="btn btn-danger">Hapus</a>
                                        </div><!-- .modal-footer -->
                                    </div><!-- /.modal-content -->
                                </div><!-- /.modal-dialog -->
                            </div><!-- /.modal --> 
            ...
            <div class="col-md-10">
                ...
                <div id="contacts-list" class="row">
                    @foreach ($suratwisatas as $suratwisata)
                    <div class="col-sm-6">
                        <div class="card user-card contact-item p-md">
                            <div class="media">
                                ...
                                <div class="media-body">
                                    <h5 class="media-heading title-color">{{ $suratwisata->name }}</h5>
                                    <small class="media-meta">{{$suratwisata->description}}</small>
                                </div>
                            </div>
                            <div class="contact-item-actions">
                                @can('sotk.edit')
                                <a href="javascript:void(0)" class="btn btn-success" data-toggle="modal"
                                    data-target="#editsuratsotk{{ $suratwisata->id }}"><i class="fa fa-edit"></i></a>
                                <a href="javascript:void(0)" class="btn btn-danger" data-toggle="modal"
                                    data-target="#deletesuratsotk{{ $suratwisata->id }}"><i class="fa fa-trash"></i></a>
                                @endcan
                                <a href="{{ asset($suratwisata->path_file) }}" target="blank" class="btn btn-warning"
                    ...
                    <!-- edit surat  Modal -->
                    <div id="editsuratsotk{{ $suratwisata->id }}" class="modal fade" tabindex="-1" role="dialog">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                ...
                                <form action="{{ route('admin.wisata.update', $suratwisata->id) }}" id="newCategoryForm" method="post" enctype="multipart/form-data">
                                    @method("put")
                                    @csrf
                                    <div class="modal-body">
                                        <div class="form-group">
                                            <input name="name" type="text" id="contactName" class="form-control" placeholder="Name" value="{{ $suratwisata->name }}">
                                        </div>
                                        <div class="form-group">
                                            <textarea name="description" class="form-control" placeholder="Deskripsi Surat">{{ $suratwisata->description }}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <select name="category" class="form-control" required>
                                                <option value="" selected disabled>Pilih Kategory</option>
                                                @foreach ($categories as $category)
                                                <option value="{{$category->id}}" @selected($category->id == $suratwisata->category_id)>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <input name="file" type="file" class="form-control" placeholder="File"><br>
                                            <a href="{{ asset($suratwisata->path_file) }}" target="blank" class="btn btn-warning" style="width: 30%"
                                                ><i class="fa fa-eye"></i></a>
                                        </div>
                                    </div><!-- .modal-body -->
                                    <div class="modal-footer">
                    ...
                    {{-- delete surat modal --}}
                    <div id="deletesuratsotk{{ $suratwisata->id }}" class="modal fade" tabindex="-1" role="dialog">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                ...
                                <div class="modal-body">
                                    <h5>hapus surat {{ $suratwisata->name }}</h5>
                                </div><!-- .modal-body -->
                                <form class="modal-footer" action="{{ route('admin.wisata.destroy', $suratwisata->id) }}" method="post">
                                    @method("delete")
                                    @csrf
...
<div id="categoryModal" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.wisata.category.store') }}" id="newCategoryForm" method="post">
                @csrf
                <div class="modal-body">
                    <div class="form-group m-0">
